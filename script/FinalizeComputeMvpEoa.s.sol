...
// ╔══════════════════════════════════════════════════════════════════════════╗
// ║  EOA-ONLY FINALIZE SCRIPT                                               ║
// ║                                                                          ║
// ║  Use this ONLY when MULTISIG is an Externally Owned Account (EOA).      ║
// ║  If MULTISIG is a Gnosis Safe contract wallet, vm.startBroadcast()      ║
// ║  cannot sign on its behalf — generate calldata for Safe UI instead:     ║
// ║    npm run finalize:compute:safe-calldata > safe-finalize-compute.json  ║
// ╚══════════════════════════════════════════════════════════════════════════╝
//
// Finalize script — called by the MULTISIG EOA to accept ownership of all four
// contracts after DeployComputeMvp.s.sol has been run.
//
...
//
// Run with:
//   forge script script/FinalizeComputeMvpEoa.s.sol:FinalizeComputeMvpEoa \
...
contract FinalizeComputeMvpEoa {
    ...
    function run() external {
        ...
        // MULTISIG here refers to the EOA private key holder.
        address multisig = vm.envOr("MULTISIG", msg.sender);

        // Contract addresses — set via env vars after deploy.
