...
fn validate_private_key(key_str: &str) -> Result<(bitcoin::secp256k1::SecretKey, bitcoin::key::PublicKey), String> {
        ...
        if let Ok(bytes) = hex::decode(trimmed) {
            if bytes.len() == 32 {
                if let Ok(secret_key) = bitcoin::secp256k1::SecretKey::from_slice(&bytes) {
                    let secp = bitcoin::secp256k1::Secp256k1::new();
                    let public_key = bitcoin::key::PublicKey::new(
                        bitcoin::secp256k1::Keypair::from_secret_key(&secp, &secret_key).public_key()
                    );
    ...
    Err("Invalid private key format. Please provide a valid WIF or 64-character hex string.".to_string())
}
...
pub fn bitcoin_create_wallet_from_private_key(
    ...
    let wallet = db.add_bitcoin_wallet(
        label,
...
pub fn bitcoin_export_mnemonic(wallet_id: String) -> Result<String, String> {
    ...
    let wallet = db.get_bitcoin_wallet(&wallet_id)
    ...
    // Get the secret
    let (secret_data, secret_type) = db.get_wallet_secret(&wallet_id)
...
pub fn bitcoin_export_private_key(wallet_id: String) -> Result<String, String> {
    ...
    // Get the secret
    let (secret_data, _) = db.get_wallet_secret(&wallet_id)
        .map_err(|e| format!("Failed to get wallet secret: {}", e))?
        .ok_or_else(|| "Wallet secret not found".to_string())?;
